...
class ChangeNameViewController: UIViewController {
    ...
    @IBAction func ChangeNameProfile(_ sender: UIButton) {
        ...
        if (NewLastText != "" && NewLastText != " " && NewFirstText != "" && NewLastText != " "){
            ...
            let dataTask =  session.dataTask(with: request) { data, response, error in
                ...
                if (APIResponse.contains("True")){
                    ...
                    DispatchQueue.main.async {
                        let alert = UIAlertController(title: "Error", message: "The Name Doesnt Exist in the System", preferredStyle: UIAlertController.Style.alert) //create alert
                        //I'm a pop up
            ...
            let alert = UIAlertController(title: "Error", message: "One or both of the fields is empty", preferredStyle: UIAlertController.Style.alert) //create alert
            //I'm a pop up
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler: nil)) // add an action (button)
            self.present(alert, animated: true, completion: nil)
        }
